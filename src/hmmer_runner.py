...
def run_hmmer(protein_sequences, output_file, threads=1):
    ...
    for hmm_file in os.listdir(hmm_model_dir):
        if hmm_file.endswith('.hmm'):
            ...
            if not os.path.exists(h3m_file):
                print(f"Running hmmpress on {hmm_file_path}", flush=True)
                cmd_hmmpress = f"hmmpress {hmm_file_path}"
                try:
                    subprocess.run(cmd_hmmpress, shell=True, check=True)
                    print(f"hmmpress completed for {hmm_file_path}", flush=True)
    ...
    for hmm_file in os.listdir(hmm_model_dir):
        if hmm_file.endswith('.hmm'):
            hmm_file_path = os.path.join(hmm_model_dir, hmm_file)
            print(f"Processing HMM file: {hmm_file_path}", flush=True)
            ...
            try:
                ...
                print(f"Completed HMM file: {hmm_file_path}", flush=True)
