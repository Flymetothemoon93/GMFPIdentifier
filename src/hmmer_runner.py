...
def run_hmmer(protein_sequences, output_file):
    ...
    # Open the output file in binary append mode ('ab') for writing results
    with open(output_file, 'ab') as f_out:
        ...
        for hmm_file in os.listdir(hmm_model_dir):
            if hmm_file.endswith('.hmm'):
                ...
                try:
                    # Run HMMER and capture the results as binary data
                    result = subprocess.run(cmd, shell=True, check=True, capture_output=True)
                    # Append the binary results to the output file
